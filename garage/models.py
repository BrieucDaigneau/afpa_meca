...
class Customer(models.Model):
    ...
    personal_data = models.OneToOneField(DonneesPersonnelles, on_delete=models.CASCADE)
...
class ReparationOrder(models.Model):
    ...
    status = models.CharField(
        ...
        default = Statut_choice[0]
    )
    user_profile = models.ForeignKey(User, on_delete=models.CASCADE, related_name="reparation_order")
...
class Supplier(models.Model):
    ...
    components = models.ManyToManyField(Component, through='Piece_Fournisseur_Devis', related_name="suppliers")
...
class Estimate(models.Model):
    ...
    def Number():
        number = Devis.objects.latest(self.id)
        return number +1

    number = models.IntegerField(unique=True, default=Number() )
    ...
    suppliers = models.ManyToManyField(Fournisseur, through='Piece_Fournisseur_Devis', related_name="estimates")
    components = models.ManyToManyField(Piece, through='Piece_Fournisseur_Devis', related_name="estimates")
