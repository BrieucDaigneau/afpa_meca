# Generated by Django 2.0.5 on 2018-07-20 06:45

import datetime
from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import garage.models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Address',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('street', models.TextField(max_length=50, verbose_name='Nom de la rue')),
                ('street_number', models.CharField(blank=True, max_length=30, null=True, verbose_name='Numéro de la rue')),
                ('street_complement', models.CharField(blank=True, max_length=50, null=True, verbose_name="Complément d'adresse")),
            ],
            options={
                'verbose_name': 'Adresse',
            },
        ),
        migrations.CreateModel(
            name='City',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('city_name', models.CharField(max_length=25, verbose_name='Ville')),
            ],
            options={
                'verbose_name': 'Ville',
            },
        ),
        migrations.CreateModel(
            name='Client',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nom_client', models.CharField(max_length=15, verbose_name='Nom Client')),
                ('prenom_client', models.CharField(max_length=15, verbose_name='Prenom Client')),
                ('numero_afpa_client', models.CharField(max_length=10, verbose_name='Numéro carte AFPA Client')),
                ('adresse', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='garage.Address')),
            ],
        ),
        migrations.CreateModel(
            name='Devis',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('numero_devis', models.IntegerField(default=garage.models.Devis.NumeroDevis, unique=True)),
                ('date_devis', models.DateField(verbose_name='Date du devis')),
                ('devis_signe_img', models.ImageField(blank=True, null=True, upload_to='img/devis', verbose_name='Scan du devis signé')),
                ('Statut', models.CharField(choices=[('VF', 'ValidationFormateur'), ('AF', 'AttenteFormateur'), ('RF', 'RefusFormateur'), ('VC', 'ValidationClient'), ('AC', 'AttenteClient'), ('RC', 'RefusClient')], default='AF', max_length=20)),
            ],
            options={
                'verbose_name_plural': 'Devis',
            },
        ),
        migrations.CreateModel(
            name='DonneesPersonnelles',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('mail_client', models.EmailField(max_length=35, verbose_name='Email Client')),
                ('telephone_client', models.CharField(max_length=10, verbose_name='Téléphone Client')),
                ('carte_AFPA_img', models.ImageField(blank=True, null=True, upload_to='img/carte_AFPA_client', verbose_name='Carte AFPA')),
            ],
        ),
        migrations.CreateModel(
            name='Fournisseur',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('libelle_fournisseur', models.CharField(max_length=35, verbose_name='Nom Fournisseur')),
            ],
        ),
        migrations.CreateModel(
            name='Intervention',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date_saisie_intervention', models.DateTimeField(default=datetime.datetime.now, null=True, verbose_name="date d'intervention")),
                ('date_restitution_prevu', models.DateField(null=True, verbose_name='Date de restitution prévisionnelle')),
                ('diagnostic', models.TextField(max_length=300, null=True)),
                ('intervention_a_realiser', models.TextField(max_length=300, null=True, verbose_name='interventions prévus')),
                ('intervention_realisee', models.BooleanField(default=False, verbose_name='intervention réalisée')),
                ('Statut', models.CharField(choices=[('VF', 'ValidationFormateur'), ('AF', 'AttenteFormateur'), ('RF', 'RefusFormateur'), ('AD', 'AttenteDevis')], default='AD', max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='Piece',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('reference_piece', models.CharField(max_length=20, verbose_name='référence pièce')),
                ('libelle_piece', models.CharField(max_length=50, verbose_name='libellé de la pièce')),
            ],
        ),
        migrations.CreateModel(
            name='Piece_Fournisseur_Devis',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('quantite_pieces_necessaires', models.IntegerField(null=True, verbose_name='Quantité de pièces nécessaires')),
                ('prix_ht', models.IntegerField(null=True, verbose_name='Prix Hors Taxes')),
                ('numero_devis_fournisseur', models.CharField(max_length=20, null=True, verbose_name='Numéro du devis fournisseur')),
                ('devis', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='garage.Devis')),
                ('fournisseur', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='garage.Fournisseur')),
                ('piece', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='garage.Piece')),
            ],
            options={
                'verbose_name': 'Commande',
                'verbose_name_plural': 'Commandes',
            },
        ),
        migrations.CreateModel(
            name='Utilisateur',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date_entree_stage', models.DateField(blank=True, null=True)),
                ('date_sortie_stage', models.DateField(blank=True, null=True)),
                ('carte_afpa', models.CharField(max_length=10, verbose_name='Numéro carte AFPA')),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='Vehicule',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('libelle_modele', models.CharField(max_length=50, verbose_name='libellé modèle')),
                ('type_vehicule', models.CharField(choices=[('VOITURE', 'Voiture'), ('MOTO', 'Moto'), ('VELO', 'Velo')], default=('VOITURE', 'Voiture'), max_length=10)),
            ],
        ),
        migrations.CreateModel(
            name='ZipCode',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('zip_code', models.IntegerField(verbose_name='Code Postal')),
            ],
            options={
                'verbose_name': 'Code Postal',
                'verbose_name_plural': 'Codes Postaux',
            },
        ),
        migrations.CreateModel(
            name='Motorise',
            fields=[
                ('vehicule_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='garage.Vehicule')),
                ('libelle_marque', models.CharField(max_length=100, null=True, verbose_name='libellé marque')),
                ('vin', models.CharField(max_length=100, null=True)),
                ('immatriculation', models.CharField(max_length=15, null=True)),
                ('kilometrage', models.IntegerField(null=True)),
                ('date_mec', models.DateField(default=datetime.datetime.now, null=True, verbose_name='date de première m.e.c.')),
                ('carte_grise_img', models.ImageField(blank=True, null=True, upload_to='img/carte_grise', verbose_name='carte grise')),
                ('carte_assurance_img', models.ImageField(blank=True, null=True, upload_to='img/carte_assurance', verbose_name='carte assurance')),
            ],
            bases=('garage.vehicule',),
        ),
        migrations.CreateModel(
            name='Velo',
            fields=[
                ('vehicule_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='garage.Vehicule')),
            ],
            bases=('garage.vehicule',),
        ),
        migrations.AddField(
            model_name='vehicule',
            name='client',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='garage.Client'),
        ),
        migrations.AddField(
            model_name='intervention',
            name='utilisateur',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='garage.Utilisateur'),
        ),
        migrations.AddField(
            model_name='intervention',
            name='vehicule',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='garage.Vehicule'),
        ),
        migrations.AddField(
            model_name='fournisseur',
            name='piece_fournisseur',
            field=models.ManyToManyField(through='garage.Piece_Fournisseur_Devis', to='garage.Piece'),
        ),
        migrations.AddField(
            model_name='devis',
            name='commande_fournisseur',
            field=models.ManyToManyField(through='garage.Piece_Fournisseur_Devis', to='garage.Fournisseur'),
        ),
        migrations.AddField(
            model_name='devis',
            name='commande_piece',
            field=models.ManyToManyField(through='garage.Piece_Fournisseur_Devis', to='garage.Piece'),
        ),
        migrations.AddField(
            model_name='client',
            name='donnees_personnelles_client',
            field=models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to='garage.DonneesPersonnelles'),
        ),
        migrations.AddField(
            model_name='city',
            name='zipCode',
            field=models.ManyToManyField(to='garage.ZipCode', verbose_name='Code Postal'),
        ),
        migrations.AddField(
            model_name='address',
            name='city',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='garage.City', verbose_name='Ville'),
        ),
        migrations.AddField(
            model_name='address',
            name='zipCode',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='garage.ZipCode', verbose_name='Code Postal'),
        ),
        migrations.CreateModel(
            name='Moto',
            fields=[
                ('motorise_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='garage.Motorise')),
            ],
            bases=('garage.motorise',),
        ),
        migrations.CreateModel(
            name='Voiture',
            fields=[
                ('motorise_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='garage.Motorise')),
            ],
            bases=('garage.motorise',),
        ),
    ]
